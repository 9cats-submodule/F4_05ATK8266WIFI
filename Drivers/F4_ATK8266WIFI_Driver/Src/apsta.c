...
u8 atk_8266_apsta_test(void)
{
	...
	u16 rlen=0;
	u8 constate=0;	//连接状态
	atk_8266_send_cmd((u8*)"AT+CWMODE=3",(u8*)"OK",50);		//设置WIFI AP+STA模式
...
//	delay_ms(1000);
	...
	atk_8266_send_cmd(p,(u8*)"OK",1000);					//设置AP模式参数
	//设置连接到的WIFI网络名称/加密方式/密码,这几个参数需要根据您自己的路由器设置进行修改!! 
	sprintf((char*)p,"AT+CWJAP=\"%s\",\"%s\"",wifista_ssid,wifista_password);//设置无线参数:ssid,密码
	atk_8266_send_cmd(p,(u8*)"WIFI GOT IP",1000);						//连接目标路由器，并获得IP
	while(atk_8266_send_cmd((u8*)"AT+CIFSR",(u8*)"STAIP",20));   //检测是否获得STA IP
	while(atk_8266_send_cmd((u8*)"AT+CIFSR",(u8*)"APIP",20));   //检测是否获得AP IP
//	while((atk_8266_consta_check()-'2'));   //获得IP
	LCD_Clear(WHITE);
	POINT_COLOR=RED;
	Show_Str(30,30,200,16,(u8*)"ATK-ESP AP+STA模式测试",16,0);
	atk_8266_send_cmd((u8*)"AT+CIPMUX=1",(u8*)"OK",50);   //0：单连接，1：多连接
	delay_ms(500);
	sprintf((char*)p,"AT+CIPSERVER=1,%s",(u8*)portnum);
	atk_8266_send_cmd(p,(u8*)"OK",50);     //开启Server模式，端口号为8086
	delay_ms(500);
	atk_8266_send_cmd((u8*)"AT+CIPSTO=1200",(u8*)"OK",50);     //设置服务器超时时间
...
PRESTA:
	...
	if(netpro&0X02)   //STA UDP
	{
				LCD_Clear(WHITE);
				POINT_COLOR=RED;
				Show_Str_Mid(0,30,(u8*)"ATK-ESP WIFI-STA测试",16,240);
				Show_Str(30,50,200,16,(u8*)"正在配置ATK-ESP模块,请稍等...",12,0);
				if(atk_8266_ip_set((u8 *)"WIFI-STA远端UDP IP设置",(u8 *)"UDP模式",(u8*)portnum,ipbuf))goto PRESTA;	//IP输入
				sprintf((char*)p,"AT+CIPSTART=0,\"UDP\",\"%s\",%s",ipbuf,(u8*)portnum);    //配置目标UDP服务器,及ID号，STA模式下为0
				LCD_Clear(WHITE);
				Show_Str_Mid(0,30,(u8*)"ATK-ESP WIFI-STA测试",16,240);
				Show_Str(30,50,200,16,(u8*)"正在配置ATK-ESP模块,请稍等...",12,0);
				atk_8266_send_cmd(p,(u8*)"OK",200);
	...
	else     //TCP
	{
		if(netpro&0X01)     //STA TCP Client  
		{
			...
			Show_Str(30,50,200,16,(u8*)"正在配置ATK-ESP模块,请稍等...",12,0);
			if(atk_8266_ip_set((u8*)"WIFI-STA 远端IP设置",(u8*)ATK_ESP8266_WORKMODE_TBL[netpro],(u8*)portnum,ipbuf))goto PRESTA;	//IP输入
			sprintf((char*)p,"AT+CIPSTART=0,\"TCP\",\"%s\",%s",ipbuf,(u8*)portnum);    //配置目标TCP服务器,及ID号，STA模式下为0
			while(atk_8266_send_cmd(p,(u8*)"OK",200))
			{
				LCD_Clear(WHITE);
				POINT_COLOR=RED;
				Show_Str_Mid(0,40,(u8*)"WK_UP:返回重选",16,240);
				Show_Str(30,80,200,12,(u8*)"ATK-ESP 连接UDP失败",12,0); //连接失败
				key=KEY_Scan(0);
	...
	LCD_Clear(WHITE);
	POINT_COLOR=RED;
	Show_Str_Mid(0,30,(u8*)"ATK-ESP WIFI-STA+AP 测试",16,240);
	Show_Str(15,50,200,16,(u8*)"正在配置ATK-ESP模块,请稍等...",12,0);
	LCD_Fill(15,50,239,50+12,WHITE);			//清除之前的显示
	Show_Str_Mid(0,50,(u8*)"WK_UP:退出 KEY0:ID0发送 KEY1:ID1发送",12,240);
	LCD_Fill(15,80,239,80+12,WHITE);
	atk_8266_get_ip(15,65);//STA+AP模式,获取 IP,并显示	
	
	
	Show_Str(15,95,200,12,(u8*)"连接状态:",12,0); 		//连接状态
	Show_Str(15,110,200,12,(u8*)"STA模式:",12,0); 		//STA连接状态
	Show_Str(120+15,110,200,12,(u8*)"AP模式:",12,0); 		//AP连接状态
	Show_Str(15,125,200,12,(u8*)"发送数据:",12,0); 	//发送数据
	Show_Str(15,140,200,12,(u8*)"接收数据:",12,0);	//接收数据
	atk_8266_wificonf_show(15,195,(u8*)"请设置路由器无线参数为:",(u8*)wifista_ssid,(u8*)wifista_encryption,(u8*)wifista_password);
	POINT_COLOR=BLUE;		
	...
	while(1)
	{
		...
		else if(key==KEY1_PRES)	//KEY1 向ID1发送数据 
		{
			  sprintf((char*)p,"ATK-8266模块ID1发数据%02d\r\n",t/10);//测试数据
				Show_Str(15+54,125,200,12,p,12,0);
				atk_8266_send_cmd((u8*)"AT+CIPSEND=1,25",(u8*)"OK",200);  //发送指定长度的数据
				delay_ms(200);
				atk_8266_send_data(p,(u8*)"OK",100);  //发送指定长度的数据
				timex=100;
		}
...
//		LCD_Fill(15+54,95,120,110,WHITE);
		...
		if(constate=='+')Show_Str(15+54,95,200,12,(u8 *)"连接成功",12,0);  //连接状态
		else Show_Str(15+54,95,200,12,(u8 *)"连接失败",12,0);
		t=0;
	}
	if((t%20)==0)LED0_T;
